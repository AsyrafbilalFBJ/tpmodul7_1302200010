...
namespace tpmodul7_1302200010 // Note: actual namespace depends on the project name.
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            obj.config.satuan_suhu = Console.ReadLine();
            ...
            if (obj.config.satuan_suhu == "celcius" && 
                suhu >= 36.5 && 
                suhu <= 37.5 && 
                perkiraan <= obj.config.batas_hari_deman)
            {
                Console.WriteLine(obj.config.pesan_diterima);
            }
            else if(obj.config.satuan_suhu == "fahrenheit" &&
                suhu >= 97.7 &&
                 suhu <= 99.5 &&
                  perkiraan <= obj.config.batas_hari_deman)
            {
            ...
            CovidConfig cov = new CovidConfig(obj.config.satuan_suhu, 
                perkiraan, 
                obj.config.pesan_diterima, 
                obj.config.pesan_diterima);
            cov.UbahSatuan();
            Console.WriteLine("Dalam "+cov.satuan_suhu+":");

            if (cov.satuan_suhu == "fahrenheit")
            {
                suhu = (suhu * 9/5) + 32;
            }
            else if (cov.satuan_suhu == "celcius")
            {
                suhu = (suhu - 32) * 5/9;
            }
